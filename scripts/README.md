...
- `run_lie_multilingual_max_audit.py`: multilingual signed-permutation and pairwise composition audit with held-out-language endpoint controls. Use `LIE_MULTI_OUT_DIR` for scale-up reruns without overwriting the canonical 2026-06-23 result; set `LIE_TRUST_REMOTE_CODE=1` for newer Hugging Face embedding models that require custom model code.
